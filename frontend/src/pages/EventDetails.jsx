...
const EventDetails = () => {
  ...
  const isCreator = () => {
    return event?.creatorId?._id === currentUser._id;
  };
          ...
          {event.attendees.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {event.attendees.map((attendee, index) => (
                <UserTooltip key={attendee._id} user={attendee}>
                  <motion.div
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: index * 0.05 }}
                    className="flex items-center gap-4 p-4 bg-gradient-to-r from-red-50 to-yellow-50 rounded-xl cursor-pointer hover:shadow-md transition-all"
                  >
                    {attendee.avatar ? (
                      <img
                        src={attendee.avatar}
                    ...
                    <div
                      ...
                      style={{ display: attendee.avatar ? 'none' : 'flex' }}
                    >
